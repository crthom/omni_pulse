const METRICS = [
  ...
  { key: 'avgWaitTime', label: 'Avg Wait Time', suffix: ' min', color: 'text-amber-400' },
...
export default function MetricsGrid({ formatted, metrics, staticComparison }) {
  ...
  const staticDelta =
    staticComparison != null
      ? Math.round((metrics.avgWaitTime - staticComparison.avgWaitTime) * 10) / 10
      : null;
  const deltaText = staticDelta === 0 ? '±0 min' : staticDelta > 0 ? `+${staticDelta} min` : `${staticDelta} min`;
